<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="description" content="SGML and XML" /> 
  <meta name="keywords" content="HTML, CSS, JAVASCRIPT" />
  <meta name="author" content="aaryan" />
  <title>welcome to SGML and XML</title>
  <link href= "styles/sgml.css" rel="stylesheet"/>
</head>

<body>

	<header>
	<img src= "images/sgml-xml-html.png" id="logo" alt="logo"/>
	<h1> INTRODUTION to SGML and XML </h1>
  <nav>
    <a href="index.php" class="menu" target="_blank" title="open index page">INTRO</a>
    <a href="topic.php" class="menu" target="_blank" title="topic">TOPIC</a>
    
    <a href="quiz.php" class="menu" target="_blank" title="QUIZ">QUIZ</a>
    <a href="enhancement.html" class="menu" target="_blank" title="enhancement">ENHANCEMENTS</a>
    <a href="enhancement2.html" class="menu" target="_blank" title="enhancement">ENHANCEMENTS2</a>
  </nav>
  </header>
  <br>
  <hr>
 	<div class="sliding" id="slideme"><ul class="sliding1"><li>Hello, welcome to my webpage. in this wepage i will tell you guys about sgml and xml and what are their similarities and differences. at the very end of the webpage there will be a quiz which you are supposed to attempt based on the material you have read on page. please rate my webpage or leave any suggestions or questions to the email id provided in the footer and sending the email is really easy just simple click the hyperlink and it will automatically open your native email application. Thankyou for visting my web page. </li></ul></div> 
	<article>
<hr>
	
	<h2> MEANING </h2>
	<h3> SGML </h3>
	<br>
	<p>
		Standard Generalized Markup Language, an international standard for defining methods of encoding electronic texts to describe layout, structure, syntax, etc., which can then be used for analysis or to display the text in any desired format.
	</p>
	<br>
	<h3> XML </h3>
	<br>
	<p>
		XML is a file extension for an Extensible Markup Language (XML) file format used to create common information formats and share both the format and the data on the World Wide Web, intranets, and elsewhere using standard ASCII text. XML is similar to HTML
	</p>
	</article>
  </header>
  <br>
  <hr>
<footer> <a href="mailto:102599490@swin.edu.au">STUDENT EMAIL : 102599490@swin.edu.au </a></footer>
</body>
</html> 
